...
class SecPlannedExpense(models.Model):
    ...
    purchase_order_total = fields.Monetary(
        string='PO Total',
        related='purchase_order_id.sec_total_mxn_manual',
        readonly=True,
        currency_field='currency_id',
    ...
    def _onchange_purchase_order_id(self):
        """Automatically import information from the selected purchase order"""
        if self.purchase_order_id:
            ...
            # Update amount - use sec_total_mxn_manual to handle currency conversions
            self.amount = po.sec_total_mxn_manual

            # Only update name if it's empty or still the default
            if not self.name or self.name in (_('New Expense'), _('New')):
                self.name = _('Purchase Order: %s') % po.name
            ...
            if description_parts:
                ...
                if self.notes:
                    ...
                    if '=== Imported from Purchase Order ===' not in self.notes:
